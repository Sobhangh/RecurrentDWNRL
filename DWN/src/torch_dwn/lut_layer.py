...
class LUTLayer(torch.nn.Module):
    def __init__(self, input_size, output_size, n, mapping='random', alpha=None, beta=None, ste=True, 
        ...
        # Residual initialization: special handling for index 1
        # luts = torch.rand(output_size, 2**n, dtype=torch.float32)*2 - 1
        # for i in range(output_size):
        #     if torch.rand(1).item() < 0.9:
        #         luts[i, 1] = (torch.rand(1, dtype=torch.float32).item()) + 0.5
        #         luts[i, [j for j in range(2**n) if j != 1]] = -torch.abs(luts[i, [j for j in range(2**n) if j != 1]])
